...
struct ALIGN(16) fvec4
{
	...
	fvec4 Normalise();
	static fvec4 Normalise(const fvec4 &v);
	flt32 Dot(const fvec4 other);
	static flt32 Dot(const fvec4 &v1, const fvec4 &v2);
	flt32 Distance(const fvec4 other);
	static flt32 Distance(const fvec4 &v1, const fvec4 &v2);
...
/**
 * @param v The vector whose normal is to be calculated
 * @return Normalised form of the given vector
 */
fvec4 fvec4::Normalise(const fvec4 &v)
{
	const __m128 ld = _mm_load_ps(v.arr);
	__m128 dp = _mm_dp_ps(ld, ld, 0xFF);
	__m128 dpsqrt = _mm_sqrt_ps(dp);
 ...
 * @return Dot product between this vector and the second vector
 */
 ...
 * @return Dot product between this vector and the second vector
 */
flt32 fvec4::Dot(const fvec4 &v1, const fvec4 &v2)
{
	__m128 m = _mm_load_ps(v1.arr);
	__m128 n =  _mm_load_ps(v2.arr);
	__m128 dot = _mm_dp_ps(m, n, 0xFF);
 ...
 * @return Returns the distance between this vector and the other vector
 */
 ...
 */
flt32 fvec4::Distance(const fvec4 &v1, const fvec4 &v2)
{
	__m128 m = _mm_load_ps(v1.arr);
	__m128 n = _mm_load_ps(v2.arr);
